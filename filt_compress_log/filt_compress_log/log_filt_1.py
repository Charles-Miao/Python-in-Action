        ...
        with os.scandir(files[0]) as entries:
            for entry in entries:
                if os.path.isfile(entry):
                    ...
                    #若指定日期，则拷贝指定日期的文件
                    elif file_change_date==date[0] or file_change_date==date[1] or file_change_date==date[2] or file_change_date==date[3] or file_change_date==date[4] or file_change_date==date[5] or file_change_date==date[6]:
                        # 在目标目录中创建修改日期目录
                        if not os.path.exists(target_folder+"\\"+file_change_date):
                            os.mkdir(target_folder+"\\"+file_change_date)  
                        # 获取源文件绝对路径
...
if __name__ == "__main__":
    source_folder=r"\\192.168.123.50\d$\Test_log"
    target_folder=r"D:\TEMP_C1"
    date=["2020-07-20","2020-07-21","2020-07-22","2020-07-23","2020-07-24","2020-07-25","2020-07-26"]
