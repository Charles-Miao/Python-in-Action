                ...
                if os.path.isfile(entry):
                    ...
                    #若指定日期，则拷贝指定日期的文件
                        ...
                        for index in range(len(date)):
                            if file_change_date==date[index]:
                                # 在目标目录中创建修改日期目录
                                if not os.path.exists(target_folder+"\\"+file_change_date):
                                    os.mkdir(target_folder+"\\"+file_change_date)  
                                # 获取源文件绝对路径
...
if __name__ == "__main__":
    ...
    for i in range(len(source_dirs)):
        p.apply_async(filt, args=(source_folder+"\\"+source_dirs[i],target_folder,source_folder_length,data_split))
    p.close()
    p.join()
