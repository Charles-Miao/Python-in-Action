...
# 2024-12-19：将StationId转换为00001~00009，上限9套治具
# ------------------------------
import re
import os
...
# 将StationId统一转换为00001~00009
def transform_string(input_string):
    for i in range(1, 10):  # 从1到9进行检查
        if f"0{i}" in input_string:
            return f"{i:05d}"  # 格式化为5位数字，前面补零
    return "NA" 
...
def process_log_file(log_file_path,xml_file_path):
    ...
    errortestname=errortestname_match.group(1) if errortestname_match else ""
    errorsuptestname=errorsuptestname_match.group(2) if errorsuptestname_match else ""
    errorfulltestname=convert_suptestname_string(errorsuptestname).strip()+"_"+errortestname if errorsuptestname_match else ""
    ...
    with open(xml_file_path+"\\"+file_name, 'w', encoding='utf-8') as output_file:
        ...
        output_file.write(f"  <StationId>{transform_string(station_id)}</StationId>\n")
        ...
        for i, match in enumerate(test_matches):
            ...
            output_file.write(f"    <TestData Name=\"Test ID {test_id} - {test_name}\" Status=\"{test_status}\" TestTime=\"{test_time_seconds}\">\n")
            ...
            for measurement in measurements_for_test:
                ...
                output_file.write(f"        <MeasurementData Name=\"{escape_xml_special_chars(measurement_name)}\" Status=\"{status}\" Result=\"{result}\" LowerLimit=\"{lowlimit}\" UpperLimit=\"{uplimit}\" Units=\"{units}\" CompType=\"{comp_type}\" LogCount=\"0\" LogTime=\"{convert_timestamp(measurement_time)}\"/>\n")
...
if __name__ == "__main__":
    ...
    log_file_path = r'C:\Users\Administrator\Desktop\EDA\Temp_Log\20241209133430276293_zone_fct_npdrearfct_NRNWXPH077C482.log'
