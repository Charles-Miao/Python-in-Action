	...
	def filt(self):
		...
		for filename in os.listdir(self.log_dir):
			...
			for index in range(len(conent)):
				if "ErrorTestName" in conent[index]:
					ErrorTestName=re.split(r'[<>]',conent[index])[2].strip()
				if "ErrorFullTestName" in conent[index]:
					ErrorFullTestName=re.split(r'[<>]',conent[index])[2].strip()
			...
			read_log.close()
			log_data.append([SN,ModelName,StationType,test_date,Result,ErrorTestName,ErrorFullTestName])

		headers=['SN','ModelName','StationType','test_date','Result','ErrorTestName','ErrorFullTestName']
